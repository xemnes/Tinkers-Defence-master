package lance5057.tDefence.core.network;
...
public class ArmorRefinerySelectionPacket extends AbstractPacketThreadsafe {

  public ArmorCore armor;
  public int activeSlots;

  public ArmorRefinerySelectionPacket() {
  }
 
  public ArmorRefinerySelectionPacket(ArmorCore tool, int activeSlots) {
    this.armor = tool;
  ...
  public void handleClientSafe(NetHandlerPlayClient netHandler) {
    ...
    if(container instanceof ArmorRefineryContainer) {
      ((ArmorRefineryContainer) container).setToolSelection(armor, activeSlots);
      if(Minecraft.getMinecraft().currentScreen instanceof ArmorRefineryGui) {
        ((ArmorRefineryGui) Minecraft.getMinecraft().currentScreen).onToolSelectionPacket(this);
      }
    }
  }
  ...
  public void handleServerSafe(NetHandlerPlayServer netHandler) {
    ...
    if(container instanceof ArmorRefineryContainer) {
      ...
      for(EntityPlayer player : server.playerEntities) {
        ...
        if(player.openContainer instanceof ArmorRefineryContainer) {
          if(((BaseContainer) container).sameGui((BaseContainer) player.openContainer)) {
            ((ArmorRefineryContainer) player.openContainer).setToolSelection(armor, activeSlots);
