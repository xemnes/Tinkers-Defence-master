...
import lance5057.tDefence.core.library.ArmorBuilder;
...
// also tool forge
public class ArmorRefineryContainer extends ContainerTinkerStation<ArmorRefineryTile> {

	private final EntityPlayer player;
	protected ArmorRefinerySlotOut out;
	...
	public ArmorRefineryContainer(InventoryPlayer playerInventory, ArmorRefineryTile tile) {
		super(tile);
		...
		// output slot
		this.out = new ArmorRefinerySlotOut(i, 124, 38, this);
	...
	protected void syncNewContainer(EntityPlayerMP player) {
		...
		TinkerNetwork.sendTo(new ArmorRefinerySelectionPacket(null, tile.getSizeInventory()), player);
	}

	@Override
	protected void syncWithOtherContainer(BaseContainer<ArmorRefineryTile> otherContainer, EntityPlayerMP player) {
		this.syncWithOtherContainer((ArmorRefineryContainer) otherContainer, player);
	}

	protected void syncWithOtherContainer(ArmorRefineryContainer otherContainer, EntityPlayerMP player) {
		...
		TinkerNetwork.sendTo(new ArmorRefinerySelectionPacket(otherContainer.selectedTool, otherContainer.activeSlots),
				player);
	...
	public void onCraftMatrixChanged(IInventory inventoryIn) {
		...
		if (!this.world.isRemote) {
			...
			for (EntityPlayer player : server.playerEntities) {
				if (player.openContainer != this && player.openContainer instanceof ArmorRefineryContainer
						&& this.sameGui((ArmorRefineryContainer) player.openContainer)) {
					((ArmorRefineryContainer) player.openContainer).out.inventory.setInventorySlotContents(0,
							out.getStack());
				}
			}
		}
