...
import lance5057.tDefence.core.library.ArmorBuildGuiInfo;
import lance5057.tDefence.core.library.TDRegistry;
import lance5057.tDefence.core.library.TDClientRegistry;
...
public class ArmorRefineryGui extends GuiTinkerStation {
  ...
  protected ArmorRefineryPreviewPanel armorPreview;

  protected ArmorRefineryGuiButtons buttons;
  ...
  public ArmorBuildGuiInfo currentInfo = ArmorRefineryGuiButtonRepair.info;

  public ArmorRefineryGui(InventoryPlayer playerInv, World world, BlockPos pos, ArmorRefineryTile armorRefineryTile) {
    super(world, pos, (ArmorRefineryContainer) armorRefineryTile.createContainer(playerInv, world, pos));

    if (!ConfigHandler.compactView) {
      armorPreview = new ArmorRefineryPreviewPanel(this, inventorySlots, 106, this.ySize - 16, new PreviewPlayer(world, Minecraft.getMinecraft().player));
      this.addModule(armorPreview);
      armorPreview.yOffset = 5;
      armorPreview.setCaption(Util.translate("gui.armorrefinery.preview"));
      armorPreview.setText();
    }

    buttons = new ArmorRefineryGuiButtons(this, inventorySlots, armorPreview != null ? 5 : 1);
  ...
  public Set<ArmorCore> getBuildableItems() {
    return TDRegistry.getArmorRefineryCrafting();
  }
  ...
  public void onToolSelection(ArmorBuildGuiInfo data) {
    ...
    ((ArmorRefineryContainer) inventorySlots).setToolSelection(tool, activeSlots);
    // update the server (and others)
    TinkerNetwork.sendToServer(new ArmorRefinerySelectionPacket(tool, activeSlots));
    updateGUI();
  }

  public void onToolSelectionPacket(ArmorRefinerySelectionPacket armorRefinerySelectionPacket) {
    ArmorBuildGuiInfo info = TDClientRegistry.getArmorBuildInfoForArmor(armorRefinerySelectionPacket.armor);
    if(info == null) {
      info = ArmorRefineryGuiButtonRepair.info;
    }
    activeSlots = armorRefinerySelectionPacket.activeSlots;
    currentInfo = info;
  ...
  public void updateDisplay() {
    // tool info of existing or tool to build
	  ArmorRefineryContainer container = (ArmorRefineryContainer) inventorySlots;
  ...
  protected void keyTyped(char typedChar, int keyCode) throws IOException {
    if(!textField.isFocused()) {
      ...
      ((ArmorRefineryContainer) container).setToolName(textField.getText());
    }
  }
  ...
  protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
    ...
    GlStateManager.scale(scale, scale, 1.0f);
    {
      ...
      if(currentInfo != null) {
        ...
        else if(currentInfo == ArmorRefineryGuiButtonRepair.info) {
    ...
    // slot logos
    if(currentInfo == ArmorRefineryGuiButtonRepair.info) {
      drawRepairSlotIcons();
    }
