...
public class TDCommands extends CommandBase implements ICommand {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] commandIn) {
		...
		if (world.isRemote) {
			...
			if (commandIn[0].equals("toggledebugmode")) {
				TinkersDefence.config.debug = !TinkersDefence.config.debug;
				if (TinkersDefence.config.debug) {
				...
				TinkersDefence.proxy.reloadRenderers();
